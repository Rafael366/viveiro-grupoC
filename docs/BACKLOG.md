...
|V-02|O critério de aceite era "o filtro estiver funcionando". Isso é subjetivo e impossível de testar.|  Definição estrita do comportamento esperado do componente, estados de seleção e mensagens de erro.|Agora exige um dropdown, atualização imediata, opção de limpar ("Todos os cursos") e mensagem para listas vazias.|
|V-03|A definição da data era confusa ("data de hoje") e ignorava o defeito B-06, onde títulos longos quebravam o layout do mural.|Ajuste da semântica da data e incorporação da correção C-06 para conter o título dentro do cartão.|A ideia passou a registrar a "data de criação" e o título foi forçado a respeitar os limites visuais do componente.|
|V-04	|Os critérios eram amadores e não testáveis ("interface amigável; busca rápida"). Além disso, não resolvia os defeitos B-01, B-02 e B-04.|	Substituição de adjetivos por comportamentos técnicos claros e incorporação das correções C-01, C-02 e C-04.|A busca tornou-se case-insensitive e ignora acentos. Foram adicionados o botão de limpar tag e a mensagem de estado vazio.|
|V-05|Assim como a V-01, a linguagem soava como rede social ("entrar num grupo") e pedia o "nome" do usuário, ferindo a Norma 1 e 3.|  Adequação do vocabulário para o ambiente acadêmico e proteção de dados fictícios.|A redação mudou para "grupo de estudo/pesquisa" e exige estritamente a exibição do "nome de usuário".|
|V-06|	O critério de aceite dizia apenas "os estados estiverem implementados", transferindo a regra de negócio para uma mera observação.	|Formalização dos três estados (Semente, Germinando, Proposta) como parte obrigatória da interface e incorporação do limite de layout (C-06).	Toda ideia nasce como Semente. |Adicionaram-se badges visuais, controle para o autor e responsividade do título no cartão.|
|V-07|Não resolvia o defeito B-05 (o contador não atualizava) e pedia para registrar o "nome" em vez do "nome de usuário". | Incorporação da correção C-05 e adequação à Norma 1 de proteção de dados.Adicionou-se a re-renderização instantânea.| O contador sobe ou desce no ato do clique, sem recarregar a página.|
